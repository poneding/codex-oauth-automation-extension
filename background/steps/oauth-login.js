    ...
    async function executeStep7(state) {
      ...
      while (attempt < STEP6_MAX_ATTEMPTS) {
        ...
        try {
          ...
          if (typeof createLocalOAuthRuntime === 'function') {
            const oauthRuntime = await createLocalOAuthRuntime();
            oauthUrl = String(oauthRuntime?.authUrl || '').trim();
            if (!oauthUrl) {
              throw new Error('本地 OAuth Runtime 未生成可用的授权地址。');
            }
            await setState({
              oauthRuntime,
              oauthUrl,
            ...
            oauthUrl = await refreshOAuthUrlBeforeStep6(currentState);
          }
