(function attachBackgroundStep3(root, factory) {
  ...
  function createStep3Executor(deps = {}) {
    ...
    async function executeStep3(state) {
      ...
      const usesCustomPassword = Boolean(state.customPassword);
      ...
      await addLog(
        `步骤 3：正在填写密码，邮箱为 ${resolvedEmail}，密码为${usesCustomPassword ? '自定义' : '自动生成'}（${password.length} 位）`
      );
      if (!usesCustomPassword) {
        await addLog(`步骤 3：本轮使用的自动生成密码为 ${password}`, 'info');
      }
