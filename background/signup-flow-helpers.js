(function attachSignupFlowHelpers(root, factory) {
  ...
  function createSignupFlowHelpers(deps = {}) {
    ...
    async function resolveSignupEmailForFlow(state) {
      ...
      if (isCustomMailProvider(state)) {
        resolvedEmail = await ensureCustomEmailForFlow(state, {
          markUsed: true,
          reuseCurrentEmail: true,
        });
        if (resolvedEmail) {
          await addLog(`步骤 2：当前自定义邮箱来源已确定为 ${resolvedEmail}。`, 'info');
        }
      }
