      ...
      for (let targetRun = resumeCurrentRun; targetRun <= totalRuns; targetRun += 1) {
        ...
        while (attemptRun <= maxAttemptsForRound) {
          ...
          try {
            ...
            const roundScopedFailure = Boolean(err?.roundScopedFailure);
            ...
            if (roundScopedFailure) {
              ...
              await appendRoundRecordIfNeeded('failed', reason);
              cancelPendingCommands('当前轮验证码流程已失败。');
              await broadcastStopToContentScripts();
              await addLog(
                targetRun < totalRuns
                  ? `第 ${targetRun}/${totalRuns} 轮验证码流程已达到上限，本轮记为失败，继续下一轮。`
                  : `第 ${targetRun}/${totalRuns} 轮验证码流程已达到上限，本轮记为失败，已无后续轮次。`,
                'warn'
              );
              forceFreshTabsNextRun = true;
              break;
