(function attachBackgroundTabRuntime(root, factory) {
  ...
  function createTabRuntime(deps = {}) {
    ...
    function getPendingQueue(source) {
      const queue = pendingCommands.get(source);
      if (Array.isArray(queue)) {
        return queue;
      }
      const created = [];
      pendingCommands.set(source, created);
      return created;
    }

    function removePendingCommand(source, target) {
      const queue = pendingCommands.get(source);
      ...
      const nextQueue = queue.filter((entry) => entry !== target);
      if (!nextQueue.length) {
        pendingCommands.delete(source);
        return;
      }

      pendingCommands.set(source, nextQueue);
    }
    ...
    function getBrowserErrorPageUrl(tab) {
      const candidates = [tab?.url, tab?.pendingUrl];
      for (const candidate of candidates) {
        const normalized = String(candidate || '').trim();
        if (isBrowserErrorPageUrl(normalized)) {
          return normalized;
        }
      }
      return '';
    ...
    function isCompatibleContentScriptSource(expectedSource, actualSource, tabUrl = '') {
      if (!actualSource || actualSource === expectedSource) {
        return true;
      }

      if (expectedSource === 'signup-page' && actualSource === 'chatgpt') {
        return matchesSourceUrlFamily('signup-page', tabUrl, 'https://chatgpt.com/');
      }

      return false;
    }

    function buildBrowserErrorPageError(source, tab) {
      const errorPageUrl = getBrowserErrorPageUrl(tab) || 'unknown';
      return new Error(
        `${getSourceLabel(source)} 页面打开失败，浏览器当前显示错误页（${errorPageUrl}）。请检查网络、代理或站点可访问性后重试。`
      );
    }

    async function assertTabReadyForInjection(source, tabId) {
      if (!Number.isInteger(tabId)) {
        return;
      }
      const tab = await chrome.tabs.get(tabId);
      if (getBrowserErrorPageUrl(tab)) {
        throw buildBrowserErrorPageError(source, tab);
      }
    }
    ...
    async function resolvePreferredWindowId() {
      const state = await getState();
      const candidate = Number(state?.ownerWindowId);
      if (Number.isInteger(candidate) && candidate > 0 && chrome?.windows?.get) {
        try {
          const win = await chrome.windows.get(candidate);
    ...
    async function ensureContentScriptReadyOnTab(source, tabId, options = {}) {
      ...
      while (Date.now() - start < timeoutMs) {
        ...
        if (pong?.ok && isCompatibleContentScriptSource(source, pong.source, currentTab?.url || currentTab?.pendingUrl || '')) {
        ...
        await assertTabReadyForInjection(source, tabId);
        ...
        if (pongAfterInject?.ok && isCompatibleContentScriptSource(source, pongAfterInject.source, currentTab?.url || currentTab?.pendingUrl || '')) {
    ...
    function getContentScriptResponseTimeoutMs(message) {
      ...
      if (message.type === 'FILL_CODE' || message.type === 'SUBMIT_VERIFICATION_CODE') {
    ...
    function resolveContentScriptResponseTimeoutMs(message, requestedTimeoutMs = undefined) {
      const automaticTimeoutMs = getContentScriptResponseTimeoutMs(message);
      const requested = Number(requestedTimeoutMs);
      if (!Number.isFinite(requested) || requested <= 0) {
        return automaticTimeoutMs;
      }
      return Math.max(automaticTimeoutMs, requested);
    }

    function resolveResilientTransportTimeoutMs(message, options = {}) {
      const recoveryAttempts = Math.max(0, Math.floor(Number(options.maxRecoveryAttempts) || 0));
      const responseTimeoutMs = resolveContentScriptResponseTimeoutMs(message, options.responseTimeoutMs);
      const requestedTimeoutMs = Number(options.timeoutMs);
      const recoveryBufferMs = Math.max(5000, recoveryAttempts * 5000);
      const minimumTimeoutMs = responseTimeoutMs + recoveryBufferMs;
    ...
    function queueCommand(source, message, timeout = 15000, options = {}) {
      return new Promise((resolve, reject) => {
        const entry = {
          message,
          resolve,
          reject,
          timer: null,
          responseTimeoutMs: resolveContentScriptResponseTimeoutMs(message, options.responseTimeoutMs),
        };
        const timer = setTimeout(() => {
          removePendingCommand(source, entry);
          reject(new Error(`Content script on ${source} did not respond in ${timeout / 1000}s. Try refreshing the tab and retry.`));
        }, timeout);
        entry.timer = timer;
        const queue = getPendingQueue(source);
        queue.push(entry);
        console.log(LOG_PREFIX, `Command queued for ${source} (waiting for ready, queued=${queue.length})`);
      });
    }
      ...
      if (Array.isArray(queue) && queue.length) {
        ...
        queue.forEach((pending) => {
          ...
          sendTabMessageWithTimeout(
            ...
            source,
            pending.message,
            resolveContentScriptResponseTimeoutMs(pending.message, pending.responseTimeoutMs)
          ).then(pending.resolve).catch(pending.reject);
        });
        console.log(LOG_PREFIX, `Flushed ${queue.length} queued command(s) to ${source} (tab ${tabId})`);
      }
    }

    function cancelPendingCommands(reason = STOP_ERROR_MESSAGE) {
      for (const [source, queue] of pendingCommands.entries()) {
        ...
        pendingCommands.delete(source);
        console.log(LOG_PREFIX, `Cancelled ${entries.length} queued command(s) for ${source}`);
      }
    }

    async function reuseOrCreateTab(source, url, options = {}) {
      const preferredWindowId = await resolvePreferredWindowId();
      const alive = await isTabAlive(source);
      if (alive) {
        const tabId = await getTabId(source);
        const currentTab = await chrome.tabs.get(tabId);
        if (Number.isInteger(preferredWindowId) && currentTab?.windowId !== preferredWindowId) {
          const registry = await getTabRegistry();
          registry[source] = null;
          await setState({ tabRegistry: registry });
        } else {
      ...
      const createPayload = { url, active: true };
      ...
      if (options.inject) {
        ...
        await assertTabReadyForInjection(source, tab.id);
        if (options.injectSource) {
          await chrome.scripting.executeScript({
            target: { tabId: tab.id },
            func: (injectedSource) => {
    ...
    async function sendToContentScript(source, message, options = {}) {
      throwIfStopped();
      const resolvedResponseTimeoutMs = resolveContentScriptResponseTimeoutMs(message, options.responseTimeoutMs);
      const registry = await getTabRegistry();
      const entry = registry[source];

      if (!entry || !entry.ready) {
        throwIfStopped();
        return queueCommand(source, message, Math.max(15000, resolvedResponseTimeoutMs), {
          responseTimeoutMs: resolvedResponseTimeoutMs,
        });
      }
      ...
      throwIfStopped();
      return sendTabMessageWithTimeout(entry.tabId, source, message, resolvedResponseTimeoutMs);
    }
    ...
    async function sendToContentScriptResilient(source, message, options = {}) {
      ...
      const resolvedResponseTimeoutMs = resolveContentScriptResponseTimeoutMs(message, responseTimeoutMs);
      const effectiveTimeoutMs = Math.max(timeoutMs, resolvedResponseTimeoutMs);
      const start = Date.now();
      let lastError = null;
      let logged = false;
      let attempt = 0;
    ...
    async function sendToMailContentScriptResilient(mail, message, options = {}) {
      ...
      const resolvedResponseTimeoutMs = resolveContentScriptResponseTimeoutMs(message, responseTimeoutMs);
      const effectiveTimeoutMs = resolveResilientTransportTimeoutMs(message, {
        timeoutMs,
        responseTimeoutMs: resolvedResponseTimeoutMs,
        maxRecoveryAttempts,
      });
    ...
    return {
      ...
      resolveContentScriptResponseTimeoutMs,
      resolveResilientTransportTimeoutMs,
      isBrowserErrorPageUrl,
      isCompatibleContentScriptSource,
