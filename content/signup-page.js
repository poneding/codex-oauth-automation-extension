...
if (document.documentElement.getAttribute(SIGNUP_PAGE_LISTENER_SENTINEL) !== '1') {
  ...
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (
      ...
      || message.type === 'SUBMIT_VERIFICATION_CODE'
...
function isChatGptAuthenticatedHomeReady() {
  ...
  if (/^\/(?:c\/|g\/|gpts(?:\/|$)|library(?:\/|$)|projects(?:\/|$)|settings(?:\/|$)|temporary-chat(?:\/|$)|discover(?:\/|$)|canvases?(?:\/|$))/i.test(pathname)) {
    return true;
  }

  const authEntryActionPattern = /log\s*in|sign\s*up|register|get\s*started|try\s*chatgpt|登录|注册|免费注册|立即注册/i;
  const authEntryActions = Array.from(document.querySelectorAll(
    'a, button, [role="button"], [role="link"], input[type="button"], input[type="submit"]'
  ))
    .filter((el) => isVisibleElement(el))
    .map((el) => getActionText(el))
    .filter(Boolean);

  if (authEntryActions.some((text) => authEntryActionPattern.test(text))) {
    return false;
  }
  ...
  if (Array.from(document.querySelectorAll(appShellSelectors)).some((el) => isVisibleElement(el))) {
    return true;
  }

  const pageText = getPageTextSnapshot();
  return /what\s+can\s+i\s+help\s+with|start\s+a\s+new\s+chat|new\s+chat|explore\s+gpts|your\s+projects|message\s+chatgpt|发送消息|新建聊天|探索\s*gpts|项目/i.test(pageText)
    && !authEntryActionPattern.test(pageText);
}
...
function inspectSignupVerificationState() {
  ...
  if (isChatGptAuthenticatedHomeReady()) {
    return {
...
async function waitForSignupVerificationTransition(timeout = 5000) {
  ...
  while (Date.now() - start < timeout) {
    ...
    if (
      ...
      || snapshot.state === 'signup_complete_home'
...
async function prepareSignupVerificationFlow(payload = {}, timeout = 30000) {
  ...
  const start = Date.now();
  let recoveryRound = 0;
  const maxRecoveryRounds = prepareSource === 'step4_transport_recovery' ? 5 : 3;
  ...
  while (Date.now() - start < timeout && recoveryRound < maxRecoveryRounds) {
    ...
    if (snapshot.state === 'signup_complete_home') {
      log(`${prepareLogLabel}：页面已直接进入 ChatGPT 首页，站点未再展示资料页，本步骤按已完成处理。`, 'ok');
      return {
        ready: true,
        alreadyVerified: true,
        directProceedToStep6: true,
        skippedProfileEntry: true,
  ...
  if (currentSnapshot.passwordInput) {
    if (!payload.password) {
      if (currentSnapshot.switchTrigger) {
        log('步骤 7：当前登录页缺少可用密码，已改为尝试切换到一次性验证码登录。', 'warn');
...
async function step5_fillNameBirthday(payload) {
  ...
  if (isChatGptAuthenticatedHomeReady()) {
    ...
    log('步骤 5：当前页面已直接进入 ChatGPT 首页，站点未再展示资料页，当前步骤按已完成处理。', 'warn');
