 ...
 * @param {Element} el
 */
function createNonSubmittingClickRestore(el, form) {
  const tagName = String(el?.tagName || '').trim().toLowerCase();
  const rawType = typeof el?.getAttribute === 'function'
    ? el.getAttribute('type')
    : null;
  const normalizedType = String(rawType || el?.type || '').trim().toLowerCase();
  const isSubmitControl = Boolean(form)
    && (
      (tagName === 'button' && (!normalizedType || normalizedType === 'submit'))
      || (tagName === 'input' && normalizedType === 'submit')
    );

  if (!isSubmitControl || typeof el?.setAttribute !== 'function') {
    return () => {};
  }

  const hadTypeAttribute = typeof el?.hasAttribute === 'function'
  ...
  return () => {
    ...
    el.setAttribute('type', rawType || 'submit');
  };
}
...
function simulateClick(el) {
  ...
  } else if (method === 'nonSubmittingClick') {
    const restoreSubmitType = createNonSubmittingClickRestore(el, form);
    try {
...
const _isMailChildFrame = SCRIPT_SOURCE === 'gmail-mail' && window !== window.top;
