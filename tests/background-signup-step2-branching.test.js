...
test('signup flow helper allocates custom email from the configured custom email pool', async () => {
  let ensureCustomEmailCalls = 0;
  let setEmailCalls = 0;

  const helpers = signupFlowApi.createSignupFlowHelpers({
    chrome: { tabs: { get: async () => ({ id: 41, url: 'https://auth.openai.com/create-account/password' }) } },
    ensureContentScriptReadyOnTab: async () => {},
    ensureCustomEmailForFlow: async () => {
      ensureCustomEmailCalls += 1;
      return 'pool-1@example.com';
    },
    isCustomMailProvider: () => true,
    isSignupEmailVerificationPageUrl: () => false,
    isSignupPasswordPageUrl: () => true,
    ...
    setEmailState: async (email) => {
      setEmailCalls += 1;
      assert.equal(email, 'pool-1@example.com');
    },
    SIGNUP_ENTRY_URL: 'https://chatgpt.com/',
    SIGNUP_PAGE_INJECT_FILES: [],
    waitForTabUrlMatch: async () => null,
  ...
  assert.equal(email, 'pool-1@example.com');
  assert.equal(ensureCustomEmailCalls, 1);
  assert.equal(setEmailCalls, 1);
});
