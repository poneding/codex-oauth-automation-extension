...
test('auto-run controller marks round-scoped verification failures as failed and continues to the next round', async () => {
  const events = {
    logs: [],
    broadcasts: [],
    accountRecords: [],
  ...
  const controller = api.createAutoRunController({
    ...
    runAutoSequenceFromStep: async () => {
      events.runCalls += 1;
      if (events.runCalls === 1) {
        const error = new Error('步骤 4：验证码被拒绝：代码不正确');
  ...
  assert.equal(events.runCalls, 2, 'round-scoped verification failure should not block the next round');
  assert.equal(events.accountRecords.length, 1);
  assert.equal(events.accountRecords[0].status, 'failed');
  assert.match(events.accountRecords[0].reason, /验证码被拒绝/);
  assert.ok(events.logs.some(({ message }) => /本轮记为失败，继续下一轮/.test(message)));
});
