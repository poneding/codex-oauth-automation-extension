...
test('step 5 skips profile form when signup already lands on chatgpt home', async () => {
  const api = new Function(`
const logs = [];
const completions = [];
...
async function waitForElement() {
  throw new Error('waitForElement should not run when profile page is skipped');
}
...
`)();
  ...
  assert.ok(snapshot.logs.some(({ message }) => /站点未再展示资料页/.test(message)));
});
